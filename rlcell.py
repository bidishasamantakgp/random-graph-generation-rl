...
class VAEGRLCell(object):
    ...
    def __call__(self, c_x, n, d, k, eps_passed, sample, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            c_x = input_layer(c_x, self.adj, self.weight, self.features, k, n, d, activation=None, batch_norm=False,
                              istrain=False, scope=None)
            ...
            with tf.variable_scope("Decoder"):
                ...
                for u in range(n):
                    ...
                    # '''
                    for v in range(n):
                        if (u,v) not in self.fixed_edges():
                            ...
                            for j in range(self.bin_dim):
                                ...
                                z_stack_weight.append(
                                    tf.concat(values=(tf.transpose(z[u]), tf.transpose(z[v]), m), axis=1)[0])
